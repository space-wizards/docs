...
-	If the anomaly is not found within a short period (due to a low player count, or the anomaly spawning in a locked room), it will likely cause substantial damage to the station and/or crew, either by going critical or via its pulse effects.
...
-	The density, distribution, and number of rifts on the station at any given point in time are up for debate and would certainly have to be fine-tuned via extensive testing. The density and quality of rifts should generally be higher in the more populated areas of the station. Certain station events could potentially increase or decrease the number of rifts currently active.
-	Rifts have different types and levels of quality - the anomaly they can generate depends on the rift type (e.g., a heat, ice, or electric anomaly could spawn from a rift with a 'forces' signature, while a flesh or floral anomaly could spawn from a rift with a 'biological' signature). Higher-quality rifts will generate anomalies with stronger pulse effects but will generate more research points. 
...
-	For example, the electrical arcs generated by an electrical anomaly can be contained by surrounding it with grounding rods; fireballs and icicles emitted by fire and ice anomalies can be degraded or destroyed if the atmosphere is sufficiently cold or hot; monsters spawned from flesh anomalies can be killed by sentry turrets or by exposure to poisonous gases; etc. Higher-quality anomalies will generally need more infrastructure to contain.
...
-	A future possibility to explore is to have contained anomalies be useful for creating certain devices/materials or provide other benefits to the station. Liquid anomalies, for example, already generate rare and useful solutions. Electrical anomalies could be used to generate power for the station; flesh anomalies could provide medical and biological reagents; etc. These benefits would increase the likelihood that players voluntarily generate the more dangerous types of anomaly.
-	Anomalous infections will also need to be reworked to account for the changes proposed here. One possibility is to have a risk of infection occurring when a player is in prolonged contact with rifts that have been marked as 'infectious' (knowingly or not). Infectious rifts would be visibly marked as such on the anomaly computer. It would be a responsibility of the science team to periodically collapse such rifts, if they don't intend to use them for research purposes, to reduce the risk of a crew member being infected. This would provide scientists with additional work to do outside of the science department, and would also give scientists a role in actively protecting the station's crew from harm. Further discussion on the matter is warranted.
...
-	The fact that rifts exist before they become anomalies and do no harm until activated (i.e., the science team does not need to race against a 'doomsday clock' in the initial stages of the gameplay loop).
...
-	Having the science department interested in space-time rifts can allow this gameplay loop to intersect with that of heretics.
