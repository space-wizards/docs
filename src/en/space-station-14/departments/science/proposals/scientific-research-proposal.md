...
-	Making all low-tier/filler researchable items available to print at round start at the protolathe and/or relevant techfabs. Examples of these items would include proximity sensors, TV circuit boards, surveillance camera equipment, etc.
-	Making the more desirable mid-tier researchable items (e.g., lasers and high-capacity batteries) also available to print at round start, but with the caveat that a technology license is required to print them. Lore-wise, this can be explained as the technology already being widely known, however, you need to purchase technology licenses from NT in order to secure the rights to print them. 
-	Scientists can exchange their research points a tech disk terminal to print these technology licenses (the amount of research points required per license depends on the item in question).
-	Licenses can then be fed into an appropriate lathe so that the item can be printed (one time for every corresponding license inserted).
