...
### Hotfix votes
Optionally, some SIMPLE/straightforward change to the PR may be suggested. Such as tweaking numbers to something **specific** not just "it should do less damage", removing some specific, concrete part, or fixing a well-defined bug/issue. It should be discussed who will/might write this fix, not just hope that someone will start writing it afterwards (yes, that has happened). It does not need to be set in stone or a ~~tribute~~ volunteer from the meeting, but we should have some idea at least.
