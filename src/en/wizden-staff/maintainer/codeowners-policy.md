...
1. **Increase velocity and turnaround time for pull requests that touch a system.**
   1. Many abandoned/derelict systems have been adopted by a maintainer that now knows the system extensively. Oftentimes, work on a system is blocked by pull requests that would normally require another maintainer to re-learn the system in order to give a proper review. This results in work being stalled for months, as learning the system is often a large effort that is not feasible to exert. The codeowners policy allows maintainers who know the system extensively to process pull requests touching those systems without requiring another maintainer.
