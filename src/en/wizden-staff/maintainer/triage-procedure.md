...
| Priority | P# | 0–3, with 0 the highest. Indicates how important the item is to the project. <br><br>**P0 – Critical:** Reserved **only** for bugfixes affecting one or more live servers that render the game unplayable (e.g., round cannot progress, major server crashes, widespread inability to perform core gameplay actions). Use with extreme caution. <br><br>**P1 – High:** Reserved for **non-critical but high-impact** bugfixes and **admin tooling**. These should be prioritized over all other PRs but are **not** emergencies. The goal is to keep the number of open P1 PRs as low as possible. **Do not assign P1 to new content.** <br><br>**P2 – Raised:** For items that are notably beneficial, useful, or deserve Maintainer attention, but don't qualify for P0/P1. <br><br>**P3 – Standard:** The default for most issues and PRs that are not urgent. |
