
## Civilian Shipyard
Largely economic vessels, accessible to anyone.

* Research (Small):
* Research (Small):
* Research, Wild (Medium):
* Research (Medium):
* Research, Wild (Large):

* Cargo (Small):
* Cargo (Small):
* Cargo (Medium):
* Cargo (Large):

* Medical (Small):
* Medical (Medium):
* Medical, Chemistry (Medium):

* Salvage (Small): DIS Notch
* Salvage (Small): DIS Vaquita
* Salvage (Medium):
* Salvage, Wild (Medium):
* Salvage, Wild (Large):

* Service (Small):
* Service (Small):
* Service (Medium):
* Service, Wild (Large):

* Engineering (Small):
* Engineering (Medium):
* Atmos (Small):
* Atmos (Medium):

* Wild (Small): Baeg
* Wild (Medium):
* Wild (Large):

* Multipurpose (Small):
* Multipurpose (Medium):
* Multipurpose (Medium):
* Multipurpose (Large):
* Multipurpose (Large):

## Medical Shipyard
Medical vessels for emergency responders and armed trauma teams.

* Medical (Small):
* Medical, Wild (Small):
* Medical, Chemistry (Small):
* Medical, Chemistry (Medium):
* Medical (Medium):
* Medical, Wild (Medium):
* Medical, Wild (Large):
* Medical, Wild (Large):

* Strafer, Medical (Small):
* Destroyer, Medical (Small):
* Destroyer, Medical (Medium):
* Cruiser, Medical, Wild (Medium):

* Engineering/Service (Medium):


## Mercenary Shipyard
Economic and combat vessels. Usually valid military targets. May include weapons with faction balancing.

* Research, Wild (Small):
* Research, Wild (Medium):

* Cargo, Wild (Small):
* Cargo, Wild (Medium):

* Medical (Small):
* Medical, Chemistry (Medium):

* Salvage, Wild (Small):
* Salvage, Wild (Medium):

* Service, Wild (Small):
* Service, Wild (Medium):

* Wild (Small):
* Wild (Medium):
* Wild (Large):

* Multipurpose (Medium):
* Multipurpose (Medium):
* Multipurpose (Large):

* Strafer (Small):
* Destroyer (Small):
* Brawler (Medium):
* Destroyer (Medium):
* Cruiser (Medium):
* Cruiser (Large):

## TSFMC Shipyard
Primarily combat vessels, exclusive to the Trans-Solarian Federation.

* Research, Salvage (Small):
* Research, Salvage (Medium):

* Multipurpose (Medium):
* Multipurpose (Large):

* Strafer (Small):
* Strafer (Medium):

* Destroyer (Small):
* Destroyer, Wild (Small):
* Destroyer (Medium):

* Brawler (Medium):

* Cruiser (Medium):
* Cruiser (Large):

* Cruiser (Capital):

## PDV Shipyard
Primarily combat vessels, exclusive to the Phaethon Dynasty.

* Research, Salvage (Small):
* Research, Salvage (Medium):

* Multipurpose (Medium):
* Multipurpose (Large):

* Strafer (Small):
* Strafer (Medium):
* Strafer (Large):

* Destroyer (Small):
* Destroyer, Wild (Small):
* Destroyer (Medium):

* Brawler (Medium):

* Cruiser (Medium):
* Cruiser (Large):

* Stealth (Small):
* Stealth (Medium):
